/*
 * LCD_integer_Button.c
 *
 * Created: 8/17/2026 1:57:04 AM
...
int main(void)
{
	...
	DDRA = 0x00;		//setting Push_Buttons for Port A as input
	PORTA = 0xFF;		//enable pull_ups for all of PORTA
	...
	while (1)
	{
		...
		itoa(PINA,ch,10); //integer to ASCII in decimal(10) format
		...
		itoa(PINA +100,ch,10); //integer to ASCII in decimal(10) format
		for (int i=0;i<3;i++) //writing character of ch array to LCD
		{
			if (ch[i] < '0' || ch[i] >'9') //if character if not a number
			{
